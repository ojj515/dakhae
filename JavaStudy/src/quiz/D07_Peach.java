...
public class D07_Peach {
	...
	public static void main(String[] args) {
		...
		peaches.add(new Peach(2600, 4.55, "A+", "김씨네농장"));
		peaches.add(new Peach(1700, 2.55, "A+", "김씨부부네농장"));
		peaches.add(new Peach(3100, 6.33, "B-", "김정묵씨네농장"));
		peaches.add(new Peach(2800, 4.55, "A", "김씨네농장"));
		peaches.add(new Peach(2800, 4.65, "C", "최씨네농장"));
		peaches.add(new Peach(2400, 7.45, "A", "박씨네농장"));
		peaches.add(new Peach(2700, 1.23, "A+", "김씨네농장"));
		peaches.add(new Peach(2100, 1.23, "B+", "김씨네농장"));
		...
		Collections.sort(peaches, new Comparator<Peach>() {
			...
			public int compare(Peach p1, Peach p2) {
				...
				if(farm1.equals(farm2)) {
					...
					// 두 농장이름 중 더 짧은 길의 문자열 길이만큼 반복해야 한다
		...
		Collections.sort(peaches, new Comparator<Peach>() {
			...
			public int compare(Peach p1, Peach p2) {
				...
				// Wapper 클래스에도 각 타입의 크기를 비교할 수 있는 메서드가
				// 미리 구현되어 있다
				if(grade1.equals(grade2)) {
