...
public class D11_ReporterList_a {
	...
	public static void main(String[] args) {
		...
		HashMap<String, List<Reporter>> categoryCompanyMap = new HashMap<>();
		...
		for(Reporter reporter : reporters) {
			...
			if(!categoryCompanyMap.containsKey(companyName)) {
				//(첫 등록일때(회사 이름이 키로 등록된 적이 없을때)
				List<Reporter> list = new ArrayList<>();
				list.add(reporter);
				categoryCompanyMap.put(companyName, list);
			} else {
				// 두 번째 이후의 등록(이미 리스트가 만들어져 있는 상태)
				List<Reporter> list = categoryCompanyMap.get(companyName);
				list.add(reporter);
			}
		}
		
		for(String company: categoryCompanyMap.keySet()) {
			System.out.println("### " + company + "의 기자들 ###");
			for(Reporter reporter: categoryCompanyMap.get(company)) {
		...
		for(Reporter reporter : reporters) {
			...
			if(!categoryfieldMap.containsKey(fieldName)) {
				List<Reporter> list = new ArrayList<>();
				list.add(reporter);
				categoryfieldMap.put(fieldName, list);
			} else {
				List<Reporter> list = categoryfieldMap.get(fieldName);
				list.add(reporter);
			}
			
		}
		for(String field: categoryfieldMap.keySet()) {
			System.out.println("### " + field + "의 기자들 ###");
			for(Reporter reporter: categoryfieldMap.get(field)) {
				System.out.println(reporter);
			}
			System.out.println();
		}
