...
public class S04_JPanel {
	...
	public static void main(String[] args) {
		...
		// 2번 패널에 붙일 버튼 생성
		ActionListener btnEventListner = new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// 이벤트가 발생할때 마다 e에 이벤트 정보가 함께 도착한다
				
				// 어떤 버튼이 이 이벤트를 발생시켰는지 getSource()를 통해 알 수 있다
				JButton btn =(JButton)(e.getSource());
				System.out.println(btn.getText() + "가 눌렀습니다");
			}
		};
		...
		for (int i = 0; i < 9; ++i) {
			...
			btn.addActionListener(btnEventListner);
			panel2.add(btn);
