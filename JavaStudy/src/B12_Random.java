...
public class B12_Random {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < 10; ++i) {
			System.out.println(ran1.nextInt(100)); // 0 ~ 99의 정수
		}
		
		// 원하는 범위의 랜덤 숫자를 생성하려면 빼기를 활용한다
		// (1) bound에 원하는 숫자의 갯수를 입력한다
		// (2) 원하는 숫자 중 가장 작은수를 더하거나 빼서 원하는 범위를 만든다
		for(int i = 0; i < 10; ++i) {
			System.out.println(ran1.nextInt(10)); // 0 ~ 9
			System.out.println(ran1.nextInt(10) - 5); // -5 ~ 4
			System.out.println(ran1.nextInt(10) + 5); // 5 ~ 14
		}
		System.out.println();
		
		//<연습> 30~60의 랜덤 숫자 30개
		for(int i = 0; i < 30; ++i) {
			System.out.println(ran1.nextInt(31) + 30); 
		}										  	
		
		// # Math.random() : 0 <= x < 1 의 실수를 랜덤으로 생성한다
		for(int i = 0; i < 10; ++i) {
