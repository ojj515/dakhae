...
public class D07_Comparator {
	 ...
	 *  # Comparable
	 *  
	 *  - 이 인터페이즈를 구현한 클래스는 크기 비교가 가능해진다
	 *  - compeareTo() 메서드를 구현해두면 크기 비교가 필요한
	 *    상황에서 해당 메서드를 호출해 크기를 비교할 수 았게 된다
 	 * 
	 * # Comparator
	 * 
	...
	public static void main(String[] args) {
	 ...
	 grapes.add(new Grape("박씨네농장",28,5.8,"B+"));
	 grapes.add(new Grape("이씨네농장",40,6.5,"A-"));
	 grapes.add(new Grape("최씨네농장",35,7.7,"A"));
	 grapes.add(new Grape("제갈씨네농장",35,7.4,"B"));
	 grapes.add(new Grape("남궁씨네농장",35,8.8,"B"));
	 grapes.add(new Grape("최씨네농장",20,8.8,"B"));
	 
	 // sort()를 할 수 있으려면 리스트의 속
	...
	// Comparator의 compare()를 사용하여 정렬한다
	 Collections.sort(grapes, new 포도분류기_당도내림차순());
	 
	 // Comparotr를 구현받은 익명의 분류기를 전달한다
...
class 포도분류기_당도내림차순 implements Comparator<Grape> {
